...
export default function DrawingsChecklist({ items, onToggle, onReset, isLoading = false }: DrawingsChecklistProps) {
  const categories = {
      ...
      <div className="space-y-4">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-6 text-center space-y-2">
            ...
            <p className="text-[11px] font-mono text-slate-400 dark:text-tokyo-muted uppercase tracking-widest animate-pulse">Extracting checklist...</p>
          </div>
        ) : items.length === 0 ? (
          <div className="text-center py-6 px-4 bg-slate-50/50 dark:bg-tokyo-input/20 border border-dashed border-slate-200 dark:border-tokyo-border rounded-lg">
            <p className="text-xs text-slate-500 dark:text-tokyo-muted leading-relaxed font-sans">
